...
class InterventionModule(nn.Module):
    '''
    if step-based:
        input: padded hs_prime (original current step) Size(1, seq_len, 4906)
        output: padded hs (intervened step)
        
    if token-based: (teacher-forcing)
        input: ground truth I+O tokens before k and self-generated k-th token
            - tensor(1, I+O length, hidden_size)
        output: ground truth k-th token
...
class Classifier():
    ...
    def forward(self, input_h):
        ...
        if self.type == 'lstm': # not aggregate input, but aggregate output (I+O length, 1)
            ...
            out, h_c = self.probe(input, h_c) # #out: Size(I+O_length, 1, output_size), h_c: Size: [2](1, 3, output_size)
            # print(f'out: {out}\nh_c.shape: {h_c}')
            assert torch.allclose(out[-1], h_c[0][-1]) #last output is the same as the last hidden state, both of shape tensor(1,1)
            # print(f'\nlstm last hidden: {h_c[0]}\n')
            sigmoid = nn.Sigmoid()
            output_h = sigmoid(h_c[0][-1]) #Size([1, 1]) #.reshape(1,1)
